...
void check1(bool check[], int x, int count_num) {
	for (int i = 0; i < 9; i++) {
		if (sedoku_table[count_num][1][x][i] != 0) {
			int where = sedoku_table[count_num][2][x][i] - 1;
			check[where] = true;
		}
	}
...
void check2(bool check[], int y, int count_num) {
	for (int i = 0; i < 9; i++) {
		if (sedoku_table[count_num][1][i][y] != 0) {
			int where = sedoku_table[count_num][2][i][y] - 1;
	...
	for (int i = 3 * xx; i < 3 * xx + 3; i++) {
		for (int j = 3 * yy; j < yy * 3 + 3; j++) {
			if (sedoku_table[count_num][1][i][j] != 0) {
				int where = sedoku_table[count_num][2][i][j] - 1;
				if (check[where] == false) {
					check[where] = true;
				}
...
void ablenum(int x , int y) {
	...
	check1(canable, numx, sedoku_quiz);
	check2(canable, numy, sedoku_quiz);
	check3(canable, numx, numy, sedoku_quiz);
	gotoxy( x , y + 8);
	for (int i = 0; i < 9; i++) {
		if (canable[i] == false) {
			cout << i + 1 << ' ';
